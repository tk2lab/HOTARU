...
def make_sim(
    ...
    for k in range(nk):
        ...
        for i in range(10):
            ...
            if (len(xs) == 0) or np.all(
                (np.array(xs) - x0) ** 2 + (np.array(ys) - y0) ** 2
                > (min_dist * radius) ** 2
            ):
                break
        if i == 9:
        ...
        ss.append(signal)
        a_true.append(
            np.exp(-0.5 * ((x - x0) ** 2 + (y - y0) ** 2) / radius**2)
        )
        u_true.append(signal * ui)
        print(k, y0, x0, radius, ui.sum(), signal)

    pd.DataFrame(dict(x=xs, y=ys, radius=rs, nspike=ts, signal=ss)).to_csv(
        "cells.csv"
    )
    nk = len(a_true)
    ...
    base_t = (
        0.5 * np.cos(np.linspace(0, 4 * np.pi, nv))[:, np.newaxis, np.newaxis]
    )
