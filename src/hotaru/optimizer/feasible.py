...
class FeasibleProxOptimizer(Module):
    ...
    def minimize(self):
        ...
        def loop(lr):
            ds = [z - lr * g for z, g in zip(self.zs, gs)]
            zns = self.model.prox(ds, lr)
            xns = [(1.0 - t) * x + t * z for x, z in zip(self.xs, zns)]
            loss_new = self.model.loss(xns)
            return zns, xns, loss_new

        def est_loss(lr):
            loss_est = loss
            for y, g, n in zip(ys, gs, xns):
                d = n - y
        ...
        self.lr.assign(lr)
        for z, x, zn, xn in zip(self.zs, self.xs, zns, xns):
            z.assign(zn)
            x.assign(xn)

        return loss_new + self.model.penalty(xns)
