...
def test_temporal():
    ...
    back_props = ComponentProperty(
        ...
        lambda v: 0.0 * np.sum(np.square(v)),
        exp_kernel(1.0, hz=data.hz),
        L2Regularizer(nonneg=True),
        lambda a: 0.0 * np.sum(np.square(a)),
    )
    props = TotalProperty(
    ...
    updater.prepare(data, cell_footprints, back_footprints, batch_size=100)
