...
def heat_fig(title, colorscale="greens", scattercolor="red", zmin=None, zmax=None):
    heat = go.Heatmap(
        ...
        zmin=zmin,
        zmax=zmax,
        colorbar={"orientation": "h", "y": 0, "yanchor": "top", "thickness": 10},
    )
    scatter = go.Scatter(mode="markers", marker_color=scattercolor)
    layout = go.Layout(
        title=title,
        xaxis={"visible": False, "autorange": False},
        yaxis={"visible": False, "autorange": False},
        width=600,
        height=570,
        margin={"t": 25, "b": 45, "l": 0, "r": 0},
    )
...
def circle_fig(title):
    scatter = go.Scatter(mode="markers", marker={"color": "green", "sizeref": 10.0})
    layout = go.Layout(
        title=title,
        xaxis={"visible": False, "autorange": False},
        yaxis={"visible": False, "autorange": False},
